...
document.addEventListener('click', function(e) {
    ...
    const isMinusIcon = e.target.classList.contains('fa-minus');
    
    if (isPlusIcon || isMinusIcon) {
        const button = e.target.closest('button') || e.target.parentElement;
        
        const container = button.closest('.quantity, .qty') || button.parentElement;
        const input = container.querySelector('input');
        
        if (input) {
            const oldValue = parseFloat(input.value) || 1;
